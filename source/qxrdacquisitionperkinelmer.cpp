/******************************************************************
*
*  $Id: qxrdacquisitionperkinelmer.cpp,v 1.2 2010/09/13 20:00:38 jennings Exp $
*
*******************************************************************/
...
QxrdAcquisitionPerkinElmer::QxrdAcquisitionPerkinElmer(QxrdDataProcessorPtr proc, QxrdAllocatorPtr allocator)
  : QxrdAcquisition(proc, allocator),
    ...
    m_CurrentGain(-1),
    m_Counter(0),
    SOURCE_IDENT("$Id: qxrdacquisitionperkinelmer.cpp,v 1.2 2010/09/13 20:00:38 jennings Exp $")
{
...
void QxrdAcquisitionPerkinElmer::initialize()
{
  ...
  nRet = this->Acquisition_EnumSensors(&nSensors, bEnableIRQ, FALSE);
  ...
  if ((nRet = this->Acquisition_GetNextSensor(&Pos, &m_AcqDesc))!=HIS_ALL_OK) {
    acquisitionNSensorsError(nRet);
    return;
  }

  if ((nRet=this->Acquisition_GetCommChannel(m_AcqDesc, &nChannelType, &nChannelNr))!=HIS_ALL_OK) {
    acquisitionError(__LINE__, nRet);
    return;
  }

  if ((nRet=this->Acquisition_GetConfiguration(m_AcqDesc, &dwFrames, &dwRows, &dwColumns, &dwDataType,
  ...
  if ((nRet=this->Acquisition_GetHwHeaderInfo(m_AcqDesc, &hwHeaderInfo)) != HIS_ALL_OK) {
    acquisitionError(__LINE__, nRet);
    return;
  }
  ...
  if (hwHeaderInfo.dwHeaderID >= 14) {
    ...
    if ((nRet = this->Acquisition_GetHwHeaderInfoEx(m_AcqDesc, &hwHeaderInfo, &hdrx)) != HIS_ALL_OK) {
      acquisitionError(__LINE__, nRet);
      return;
    }
  ...
  if ((nRet=this->Acquisition_GetIntTimes(m_AcqDesc, readoutTimes, &nReadoutTimes)) != HIS_ALL_OK) {
    acquisitionError(__LINE__, nRet);
    return;
  }
  ...
  if ((nRet=this->Acquisition_SetCallbacksAndMessages(m_AcqDesc, NULL, 0,
  ...
  if ((nRet=this->Acquisition_SetFrameSyncMode(m_AcqDesc, HIS_SYNCMODE_INTERNAL_TIMER)) != HIS_ALL_OK) {
    acquisitionError(__LINE__, nRet);
    return;
  }
  ...
  if ((nRet=this->Acquisition_DefineDestBuffers(m_AcqDesc, m_Buffer.data(), m_BufferSize,
  ...
  if ((nRet=this->Acquisition_Acquire_Image(m_AcqDesc, m_BufferSize,
